...
const StoriesBar = () => {
  ...
  return (
    <div className="w-screen sm:w-[calc(100vw-240px)] lg:max-w-2xl no-scrollber overflow-x-auto px-4">
        ...
        {stories.map((story, index) => (
          ...
          >
            <img
              ...
              className="absolute
                  size-8 top-3 left-3 z-10 rounded-full ring ring-gray-100 shadow"
              alt=""
            />
            <p className="absolute top-18 left-3 text-white/69 text-sm truncate max-w-24">
              {story.content}
            </p>
            ...
            {story.media_type != "text" && (
              <div className="absolute inset-0 z-1 rounded-lg bg-black overflow-hidden">
                {stories.media_type === "image" ? (
                  <img
                    src={story.media_url}
                    className="h-full w-full object-cover hover:scale-110 transition-all duration-500 opacity-70 hover:opacity-80"
                    alt=""
                  />
                ) : (
                  <video
